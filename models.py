    ...
    def _computePlot(self, z_data, params):
        ...
        if self.expect_z == "edges":
            if hasattr(z_data, "edges"):
                ...
                z_model = copy(z_data)
                try:
                    z_plot = (z_model[1:] + z_model[:-1]) / 2.0
                except TypeError:
                    z_plot = [(z[1:] + z[:-1]) / 2.0 for z in z_model]
        else:
            z_model = z_plot
        # call the model
        try:
            iter(params)
        except TypeError as e:  # parameters are a FitResult instance
            NotImplemented
        else:
            data = self(z_model, *params)
            errors = None
        return z_plot, data, errors

    def plot(self, z_data, params, ax=None, **kwargs):
        if ax is None:
            fig = Figure(1)
            ax = fig.axes[0]
        else:
            fig = plt.gcf()
        # make the plot
        z, data, error = self._computePlot(z_data, params)
        line = ax.plot(z, data, **kwargs)[0]
        if error is not None:
            ax.fill_between(
                z, data - error, data + error,
...
class BaseModelBinned(BaseModel):
    ...
    def plot(self, z_data, params, fig=None, **kwargs):
        fig, axes = self._getFig(fig)
        # make the plot
        z, data, error = self._computePlot(z_data, params)
        for i, ax in enumerate(axes.flatten()):
            line = ax.plot(z[i], data[i], **kwargs)[0]
            if error is not None:
                ax.fill_between(
                    z[i], data[i] - error[i], data[i] + error[i],
                    alpha=0.3, color=line.get_color())
        return fig
